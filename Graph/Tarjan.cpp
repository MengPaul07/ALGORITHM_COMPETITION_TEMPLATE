    ...
    tarjan = [&](int u,int pe){
        ...
        for(auto [v,id] : adj[u]){
            ...
            if(!dfn[v]){
                ...
                if (pe != -1 && low[v] >= dfn[u]) cut[u] = 1;
            }
            else{
                low[u] = min(low[u],dfn[v]);
            }
        }
        if (pe == -1 && child >= 2) cut[u] = 1;
    };
    for(int i = 1;i <= n;i++){
        if (!dfn[i]) tarjan(i,-1);
    }
...
// Tarjan 缩点并在 DAG 上求从 1 到 n 的最大点权路径和。
void solve(){
    int n,m;cin >> n >> m;
    vector<int> a(n + 1);
...
void solve(){
    ...
    for(int i = 1;i <= n;i++){
        ...
        tarjan(i,-1);
        // 根节点不会被上面的条件弹出
        if (!st.empty() && st.back() == i) st.pop_back();
        // 孤立点，或者只有自环的点
